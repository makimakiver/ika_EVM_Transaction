...
function splitRSV(raw) {
    ...
    if (raw.length === 64) {
        const r = ethers.hexlify(raw.slice(0, 32));
        const s = ethers.hexlify(raw.slice(32, 64));
        return { r, s, v: undefined };
    }
    throw new Error(`Unexpected signature length: ${raw.length}`);
}
export async function broadcastSignedTx(provider, populatedTx, unsignedBytes, rawSig, expectedFrom) {
    ...
    if (v === undefined) {
        ...
        if (v === undefined)
            throw new Error("Could not determine v");
    }
    ...
    if (populatedTx.nonce != null)
        txData.nonce = populatedTx.nonce;
    if (populatedTx.gasLimit != null)
        txData.gasLimit = populatedTx.gasLimit;
    if (populatedTx.gasPrice != null)
        txData.gasPrice = populatedTx.gasPrice;
    if (populatedTx.data != null)
        txData.data = populatedTx.data;
    if (populatedTx.value != null)
        txData.value = populatedTx.value;
    if (populatedTx.chainId != null)
        txData.chainId = populatedTx.chainId;
    if (populatedTx.type != null)
        txData.type = populatedTx.type;
    if (populatedTx.accessList != null)
        txData.accessList = populatedTx.accessList;
    if (populatedTx.maxFeePerGas != null)
        txData.maxFeePerGas = populatedTx.maxFeePerGas;
    if (populatedTx.maxPriorityFeePerGas != null)
        txData.maxPriorityFeePerGas = populatedTx.maxPriorityFeePerGas;
